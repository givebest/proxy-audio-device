...
class ProxyAudioDevice {
  public:
    ...
    // Must be called while holding stateMutex / stateMutex2
    void updateCachedVolumeFactors();
    void updateCachedVolumeFactors2();
    ...
    // Precomputed amplitude factors; updated whenever volume or mute changes.
    // Written under stateMutex, read lock-free in the IO proc.
    std::atomic<Float32> gVolumeFactorL{1.0f};
    std::atomic<Float32> gVolumeFactorR{1.0f};
    ...
    std::atomic<Float32> gVolumeFactorL2{1.0f};
    std::atomic<Float32> gVolumeFactorR2{1.0f};
};
